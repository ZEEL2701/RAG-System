...
class SessionBasedRAG:
    ...
    def index_document(self, file_path: str) -> bool:
        ...
        try:
            ...
            if not file_info or file_info.get("file_id") is None:
                logger.error(f"Failed to store file: {file_path}")
                return False

            if file_info.get("duplicate_skipped"):
                logger.info(f"Duplicate upload skipped (already stored for session): {file_path}")
                return True
